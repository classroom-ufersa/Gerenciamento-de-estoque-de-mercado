...
Produto *retira(Produto *l, char nome_produto[])
{
    ...
    while (p->nome != nome_produto)
    {
        ...
        p = p->prox;
        if (ant == NULL)
        { // verifica se o nó a ser retirado é p primeiro da lista
            l = p->prox;
        }
        else
        {
...
Produto *ler_arquivo(Produto *produto, int *tam)
{
    ...
    while(fscanf(arquivo,"%99[^] %99[^] %f %99[^] %99[^] %99[^] %99[^] %d", lista_produto->tipo, lista_produto->nome, &lista_produto->preco, lista_produto->data_fabricacao, lista_produto->data_validade, lista_produto->departamento->nome, lista_produto->departamento->porte, &lista_produto->estoque))
